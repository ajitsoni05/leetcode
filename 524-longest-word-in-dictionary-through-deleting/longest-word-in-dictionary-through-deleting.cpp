...
public:
    ...
    string findLongestWord(string s, vector<string>& dictionary) {
        ...
        iterate in the loop and check for other words of greater length, then
        compare
        ...
        for (int i = 0; i < dLen; i++) {
            ...
            if (word.size() >= maxLen) {
                ...
                if (isSubsequence(s, word)) {
                    ...
                    if (cand == "" || word.size() > cand.size() ||
                        lexicographical_compare(word.begin(), word.end(),
                                                cand.begin(), cand.end())) {
                        cand = word;
